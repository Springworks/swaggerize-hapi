...
function buildValidators(validators) {
    ...
    if (!validate.payload && formValidators) {
        validate.payload = function (value, options, next) {
            ...
            Async.series(keys.map(function (k) {
                return function (callback) {
                    try {
                        if (typeof formValidators[k] !== 'function') {
                            callback(new Error('Form validator for key "' + k + '" is not a function. Available keys: ' + keys.join(', ')));
                            return;
                        }
